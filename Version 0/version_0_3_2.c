...
void executa_prior (struct processo **prior){
	int quantum = 3; //Definição do valor do quantum para teste
	if (!(*prior == NULL)){ //Verifica se a lista está vazia
	    ...
	    if ((*prior)->tempo <= 0){ //Remove o processo da lista se o tempo do processo zerar
		    printf ("%s da lista de %s foi finalizado!\n\n", (*prior)->rotulo, (*prior)->tipo_lista);
		    remover (prior);
	    }
...
void executa_fifo (struct processo **fifo){
	if (!(*fifo == NULL)){ //Verifica se a lista está vazia
		(*fifo)->tempo = 0; //Realiza a "execução" do processo de acordo com o escalonamento FIFO (não preemptivo)
		printf ("%s da lista %s foi finalizado!\n\n", (*fifo)->rotulo, (*fifo)->tipo_lista);
...
void alterna_pronto (struct processo **fifo, struct processo **prior, int cont_exec){
...
int main(void) {
  ...
  do {
    ...
    switch (op) {
      ...
      case 3:
      	cont_exec++;
      	alterna_pronto (&fifo, &prior, cont_exec);
      	break;
		case 5:
			cont++;
			if(ver_interrupcao()){
