...
/* Mateus Marques Montagnoli, João Pedro Quadrado, Fabrício Gustavo Frank */
/**************************************************************************/
...
struct processo {
  ...
  int id, tempo, prioridade; // identificador, tempo necessário de processamento e nivel de prioridade (entre 0 e 5)
  struct processo *prox; // ponteiros para o proximo elemento
};

//funcao que faz a inserção de elementos nas Listas Principais(RoundRobin e Prioridade)
void inserir(struct processo **inicio, int prioridade, int id, int tempo, int tipo, char rotulo[]) {
  ...
  if (tipo) { //se o tipo for 1,o elemento será inserido na lista do metodo RoundRobin
    // Inicio da inserção por roundrobin
      ...
      if (novo->prioridade >= (*inicio)->prioridade) { // Inicio da fila
        ...
        *inicio = novo;
        ...
        if (aux == NULL) { // Menor prioridade, fim da lista
          anterior->prox = novo;
          novo->prox = NULL;
        } else { // Prioridade intermediária, no meio da lista
...
// Função que decide em qual lista o processo gerado irá entrar
void separar_processos(struct processo **rr, struct processo **prior, char rotulo[], int id, int tempo, int prioridade) {
  if (tempo >= 7) {//caso tempo muito alto(maior ou igual a 7),o processo é inserido na lista do método RR
    printf("O processo %s foi colocado na FILA PELO METODO ROUNDROBIN\n",rotulo);
    inserir(rr, prioridade, id, tempo, 1, rotulo);
  } else if (tempo < 7) { //senão,o processo é inserido na lista  por prioridade
...
// Função que faz a remoção de um processo na lista
...
// Função que faz a inserção de elementos na Lista de Dispositivos
// Recebe como parâmetros os valores do processo que anteriormente estava armazenado na fila pronto
void InserirDisp(struct processo **disp,int prioridade, int id, int tempo, char tipo_lista[], char rotulo[] ){
  ...
  aux = *disp;
  novo = malloc(sizeof(struct processo)); //alocação dinâmica da memória
  novo->prioridade = prioridade; //Atribuindo os valores dos parâmetros no ponteiro novo
  novo->id = id;
  novo->tempo = tempo;
  ...
  if(*disp == NULL){ //verifica se a lista de dispositivo esta vazia
    ...
    *disp = novo;
    ...
    while(aux != NULL) {//percorre até o ultimo elemento da lista
      anterior = aux;
      aux = aux->prox;
...
// Função que cria os programas junto com a geração de processos
void cria_programa(int *program_id, int *process_id, struct processo **rr, struct processo **prior){
  FILE *programa; // Cariável FILE que será usada para manipulações em arquivos
  char filename[100], process_name[100], linha[150];
  int i, qtd_processos;
  int id, tempo, prioridade;

  // Geração do nome do arquivo que será criado
  ...
  if (programa != NULL) { // Verifica se o arquivo pode ser aberto
    srand(time(NULL));
    qtd_processos = rand()%4 + 1;// Geração aleatória da quantidade de processos que o programa terá
    printf("\n\nForam gerados %i processos neste programa!Abaixo se encontra os processos criados:\n\n",qtd_processos);
    for(i = 0; i < qtd_processos; i++) { // Criação dos processos do programa
      // Gera o nome do processo
      sprintf(process_name, "Processo_%02d_PROG_%02d", *process_id,*program_id);
      // Gera aleatoriamente o tempo total de execução do processo e sua prioridade
      fprintf(programa,"Processo:%s ID:%d Tempo:%d Prioridade:%d\n", process_name, *process_id, rand()%15 + 1, rand()%6);
      (*process_id)++;
    }
    fclose(programa); // Fecha o arquivo aberto do modo "append" para poder abrí-lo em outro modo
    (*program_id)++;
    // Carrega n processos
    if((programa = fopen(filename, "r")) != NULL) { // Tenta abrir o arquivo do programa no modo leitura
      while(fgets(linha, sizeof(linha), programa)) { // Lê o arquivo linha a linha até que chegue ao final
        ...
        // Insere o processo carregado em uma das listas para futura execução;
        separar_processos(rr, prior, process_name, id, tempo, prioridade);
      }
      fclose(programa); // Fecha o arquivo que não será mais utilizado
      ...
      exit(EXIT_FAILURE); // Finalização do programa em caso de erro
    }
...
void imprime_lista(struct processo *lista) {
  ...
  while (aux != NULL) {// Percorre a lista até o ponteiro aux ficar nulo,imprimindo todos os valores
...
// função que insere o processo interrompido na lista de dispositivos e remove da lista em que estava
...
// função que retorna o processo que se encontra na fila de dispositivos para uma das filas pronto
void retornar_processoDisp(struct processo **rr, struct processo **prior, struct processo **lista_disp){
  if(!(*lista_disp == NULL)){ // // Verifica se a lista está vazia
    ...
    else // Senão o processo retornando era da lista do metodo round robin
...
// função que gera aleatoriamente um número de 1 a 10. Se o numero for maior que 6,o processo será interrompido, senão ,nao será
...
// função que executa um processo que está na lista de prioridades
void executa_prior (struct processo **prior, struct processo **disp){
  int quantum = 3; // Definiçãoo do valor do quantum

  if(gerar_interrupcao()){ // Chamada da função que decide se ocorre interrupção
    fazer_interrupcao(prior , disp); // Realização da interrupção
  }
    ...
    else{ /* Se o tempo não zerar realiza a cópia das informações do processo para uma variável auxiliar e insere
  ...
  {
    printf("\nTodos os processos estao na FILA DE DISPOSITIVOS!\nGere mais programas ou execute ate que os processos voltem para suas filas\n\n");
  }
}

// função que executa um processo que esta na lista pelo metodo RoundRobin
void executa_rr (struct processo **rr, struct processo **disp){
  int quantum = 5; //Definição do valor do quantum

  if(gerar_interrupcao()){ // Chamada da função que decide se ocorre interrupção
    fazer_interrupcao(rr , disp); // Realização da interrupção
  }
  else if (!(*rr == NULL)){ // Verifica se a lista está vazia
...
// função que alterna a execução das duas filas pronto, executando 4x a lista de prioridades e 1x a lista pelo metodo rr
...
// função que permite a criação da interface de carregamento, como aparece "CRIANDO PROGRAMA" e "IMPRIMINDO" usando a função sleep
...
int main(void) {
  ...
  int cont_inter = 0;
  int cont_exec = 0;
  char c; // Variável auxiliar para tratamento de entradas não desejadas no scanf
  ...
  do {
    ...
    do {
      ...
      // Limpa o resto da entrada não lida do buffer para evitar erros
      while((c = getchar()) != '\n' && c != EOF);
    } while(op < 1 && op > 4);
    printf("\n");

    switch (op) { // switch case para separar as opcoes do usuario
      case 1: // se o usuario digitou 1,será gerado um novo programa com varios processos
        interface_loading(1);
        cria_programa(&id_programa, &id_processo, &rr, &prior);
        printf("\nPrograma criado com sucesso!\n\n");
        break;
      case 2: // se o usuario digitou 2,será impresso todos os processos
        interface_loading(2);
        printf ("--------------\n");
        ...
        printf ("--------------\n");
        break;
      case 3: // se o usuario digitou 3, o próximo processo será executado
        cont_exec++;
        cont_inter++;
